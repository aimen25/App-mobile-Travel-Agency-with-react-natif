...
export default function SortCategories({ setActiveCategory }) {
  const [activeSort, setActiveSort] = useState('Sans visa');
    ...
    <View className="flex-row justify-around items-center mx-4 bg-neutral-100 rounded-full p-2 px-4 space-x-2">
      {sortCategoryData.map((sort, index) => {
        ...
        return (
          ...
          >
            <Text className="font-semibold" style={{fontSize: wp(4), color: isActive ? theme.text : 'rgba(0,0,0,0.9)'}}>{sort}</Text>
          </TouchableOpacity>
        );
      })}
    </View>
